...
echo $PATH_HARD_SYSTEMS_PKG_4DATA
if [ ! -d "${PATH_HARD_SYSTEMS_PKG_4DATA}" ]
then
    echo "[PyTorch-Quantization] PyTorch-Quantization data storage is not properly configured!"
else
    ...
    ln -s ${PATH_HARD_PROBLEM_PKG_LOGS} ${PATH_PROBLEM_PKG}/logs
